...
CURRENT_PAGE = "code_breaker"

if (
    "current_page" not in st.session_state
    or st.session_state.current_page != CURRENT_PAGE
):
...
def new_game():
    ...
    st.session_state.color_options = selected_labels          # dùng cho selectbox
...
def evaluate_guess(guess, secret):
    ...
    remaining_guess = [
        g for g, s in zip(guess, secret)
        if g != s
    ]

    remaining_secret = [
        s for g, s in zip(guess, secret)
        if g != s
    ]

    color_only = sum(
        min(
            remaining_guess.count(color),
            remaining_secret.count(color),
        )
...
config_col, play_col = st.columns(
    [2, 3],
    gap="large",
)
...
with play_col:
    ...
    tries_left = (
        st.session_state.max_tries - tries_used
    )
    st.write(" ")
    st.markdown("""
