...
def ensure_current_page():
    """Đảm bảo current_page đúng"""
...
# ────────────────────────────────────────────────
def run():
    """Hàm chính để chạy game Tower of Hanoi"""
    
    # Reset state khi chuyển tab
    ensure_current_page()
    
    # Set page config (chỉ có tác dụng khi chạy standalone)
    st.set_page_config(
    ...
    # Instructions với images (chỉ hoạt động khi chạy standalone)
    with st.expander("Instruction", expanded=True):
        cols = st.columns(3, gap="large")
        for i, col in enumerate(cols, start=1):
            img_path = Path(__file__).parent / "assets" / "tower_of_hanoi" / f"hint{i}.png"
            with col:
                if img_path.exists():
                    st.image(img_path)
                else:
                    st.caption(f"Hint {i} (image not found)")


# ────────────────────────────────────────────────
# Chạy khi execute trực tiếp
# ────────────────────────────────────────────────
