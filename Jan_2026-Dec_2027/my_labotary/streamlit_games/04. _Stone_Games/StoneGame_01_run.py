...
play_col, hist_col = st.columns([2, 1], gap='medium')
with play_col:
    col_left, col_right, newgame = st.columns(3)
    col_left.metric("**:blue[You (Alice)]**", st.session_state.you_score)
    col_right.metric("**:green[Computer (Bob)]**", st.session_state.computer_score)
    with newgame:
    ...
    if st.session_state.game_over:
        ...
        st.subheader("**:blue[Các viên đá còn lại]**")
        # Hiển thị theo grid n//2 cột
        n_cols = len(st.session_state.nums) # // 2
                ...
                if idx < len(st.session_state.nums):
                    col.markdown(
                        ...
                        <div style="
                            ...
                            border-radius: 10px;
                            color: white;
                            background: radial-gradient(circle, #4F46E5 35%, transparent 65%);
                            font-size: 20px;
                            font-weight: 600;">
...
with hist_col:
    with st.expander("**:blue[Lịch sử lượt chơi]**", expanded=True):
        if st.session_state.history:
            col_you, col_bot = st.columns(2)
            with col_you:
                st.markdown("""
                            <div style='font-weight:bold; color:blue; font-size: 25'> 
                                You (Alice) 
                            </div>""", 
                            unsafe_allow_html=True)
            ...
            with col_bot:
                st.markdown("""
                            <div style='font-weight:bold; color:green; font-size: 25'> 
                                Computer (Bob) 
                            </div>""", 
                            unsafe_allow_html=True)
                for i, item in enumerate(st.session_state.history, start=1):
