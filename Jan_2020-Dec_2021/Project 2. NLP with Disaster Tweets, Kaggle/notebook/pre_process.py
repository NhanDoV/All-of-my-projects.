...
def Ngrams_generate(DISASTER_TWEETS, df_train, ngram):
    """
        Generate Ngrams dataset for both disaster vs non-disaster
    """
    disaster_Ngrams = defaultdict(int)
    nondisaster_Ngrams = defaultdict(int)

    for tweet in df_train[DISASTER_TWEETS]['text']:
        for word in generate_ngrams(tweet, n_gram=ngram):
    ...
    df_disaster_trigrams = pd.DataFrame(sorted(disaster_Ngrams.items(), key=lambda x: x[1])[::-1])
    df_nondisaster_trigrams = pd.DataFrame(sorted(nondisaster_Ngrams.items(), key=lambda x: x[1])[::-1])

    return df_disaster_trigrams, df_nondisaster_trigrams

def ngram_show(df_disaster_Ngrams, df_nondisaster_Ngrams, n_gram, top_N, figsize=(18, 50)):
    """
        Show barplot by count the topN of Ngrams in both disaster vs non-disaster dataset
    """
    if n_gram == 1:
        ng_str = "unigrams"
    elif n_gram == 2:
        ...
        ng_str = f"{n_gram}-grams"

    fig, axes = plt.subplots(ncols=2, figsize=figsize, dpi=100)
    plt.tight_layout()

    sns.barplot(y=df_disaster_Ngrams[0].values[:top_N], 
                x=df_disaster_Ngrams[1].values[:top_N], 
                ax=axes[0], color='red')
    sns.barplot(y=df_nondisaster_Ngrams[0].values[:top_N], 
                x=df_nondisaster_Ngrams[1].values[:top_N], 
    ...
    axes[0].set_title(f'Top {top_N} most common {ng_str} in Disaster Tweets', fontsize=15)
    axes[1].set_title(f'Top {top_N} most common {ng_str} in Non-disaster Tweets', fontsize=15)
    plt.show()
